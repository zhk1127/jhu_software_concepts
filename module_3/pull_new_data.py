...
def scrape_new_records():
    ...
    print(f"Starting scrape from page {START_PAGE}", flush=True)
    ...
    while len(all_records) < TARGET_RECORDS:
        ...
        cleaned_page_records = [clean_record(record) for record in page_records]
...
def main():
    ...
    print(f"Saved {len(raw_records)} records to {RAW_OUTPUT_FILE}", flush=True)

    cleaned_records = clean_data(raw_records)

    save_json(cleaned_records, CLEAN_OUTPUT_FILE)
    print(f"Saved {len(cleaned_records)} records to {CLEAN_OUTPUT_FILE}", flush=True)

    inserted, skipped = insert_records_into_database(cleaned_records)

    print(f"Inserted {inserted} new records into PostgreSQL.", flush=True)
    print(f"Skipped {skipped} duplicate or invalid records.", flush=True)
